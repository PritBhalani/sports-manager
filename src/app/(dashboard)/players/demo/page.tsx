...
export default function DemoPlayersPage() {
  ...
  const rows: PlayerRow[] = useMemo(
    () =>
      Array.from({ length: 18 }).map((_, idx) => {
        ...
        return {
          id: `DM-${n}`,
          username: `demo_player_${n}`,
          userCode: `D${30000 + n}`,
          status: "Demo",
          createdAt: makeIsoDate(n + 2),
        };
      }),
    [],
  );
  ...
  return (
    <div className="min-w-0 space-y-4 sm:space-y-6">
      <PageHeader title="Demo Players" breadcrumbs={["Reports", "Demo Players"]} />

      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <StatsCard title="Total Demo" value={String(rows.length)} />
        <StatsCard title="Active Demos" value={String(rows.length)} />
        <StatsCard title="Converted (Demo)" value="0" />
        <StatsCard title="Latest Demo" value={formatDateTime(rows[0]?.createdAt ?? "—")} />
      </div>

      <ListPageFrame>
        <div className="flex w-full flex-col justify-center gap-0">
          <FilterBar className="rounded-none bg-neutral-200 px-5 pb-4 pt-4">
            <Input placeholder="Search demo players" className="max-w-xs" />
            <Button variant="primary" type="button" onClick={exportDemoCsv}>
              Export
            </Button>
          </FilterBar>
          <ListTableSection>
            <DataTable
              ...
              rows={rows}
              columns={columns}
              emptyMessage="No demo players found."
            />
          </ListTableSection>
        </div>
      </ListPageFrame>
