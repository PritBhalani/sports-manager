...
export default function InactivePlayersPage() {
  ...
  const exportInactiveCsv = useCallback(() => {
    downloadCsv(
      "inactive-players.csv",
      ["Username", "User code", "Status", "Registered"],
      data.map((row) => [
        String(row.username ?? ""),
        String(row.userCode ?? ""),
        String(row.status ?? ""),
        ...
        <div className="flex w-full flex-col justify-center gap-0">
          ...
          <TableBody>
            {loading ? (
              ...
              <TableEmpty colSpan={4} message="No inactive players." />
            ) : (
              data.map((row, i) => (
                <TableRow key={i}>
