...
export default function FTDPlayersPage() {
  ...
  const rows: PlayerRow[] = useMemo(
    () =>
      Array.from({ length: 18 }).map((_, idx) => {
        ...
        return {
          id: `FTD-${n}`,
          username: `ftd_user_${n}`,
          userCode: `F${50000 + n}`,
          status: "FTD",
          createdAt: makeIsoDate(n + 3),
        };
      }),
    [],
  );
  ...
  return (
    <div className="min-w-0 space-y-4 sm:space-y-6">
      <PageHeader title="FTD" breadcrumbs={["Reports", "FTD"]} />

      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <StatsCard title="First Time Depositors" value={String(rows.length)} />
        <StatsCard title="Verified FTD" value={String(Math.floor(rows.length * 0.7))} />
        <StatsCard title="Unverified" value={String(rows.length - Math.floor(rows.length * 0.7))} />
        <StatsCard title="Latest FTD" value={formatDateTime(rows[0]?.createdAt ?? "—")} />
      </div>

      <ListPageFrame>
        <div className="flex w-full flex-col justify-center gap-0">
          <FilterBar className="rounded-none bg-neutral-200 px-5 pb-4 pt-4">
            <Input placeholder="Search FTD users" className="max-w-xs" />
            <Button variant="primary" type="button" onClick={exportFtdCsv}>
              Export
            </Button>
          </FilterBar>
          <ListTableSection>
            <DataTable
              ...
              rows={rows}
              columns={columns}
              emptyMessage="No FTD players found."
            />
          </ListTableSection>
        </div>
      </ListPageFrame>
